...
OpenAI-compatible endpoint using the paper's benchmark suites.
...
`--ignore-requires-python` is required: the `[eval]` extra pulls
...
- **AIME.** The suite reads `${WORKSPACE_DIR}/aime-<year>/aime-<year>.jsonl`
  (default workspace `/workspace/spade-workspace`); a `path_template` in the
